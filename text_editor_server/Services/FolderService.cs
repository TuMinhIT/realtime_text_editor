...
namespace text_editor_server.Services
{
    public class FolderService
    {
        ...
        // download folders nén zip lại luon 
        public async Task<ServiceResult<ProofFileDownloadRes>> DownloadProofFolderAsync(Guid folderId)
        {
            try
            {
                ...
                return ServiceResult<ProofFileDownloadRes>.Ok(
                    new ProofFileDownloadRes
                    {
            ...
            {
                _logger.LogError(ex,
                    "Failed to download folder {FolderId}",
                    folderId);

                return ServiceResult<ProofFileDownloadRes>
                    .Fail("Failed to download folder");
            }
        }
        ...
        public async Task<ServiceResult<List<FolderRes>>> GetAllFolderAsync()
        {
            var folders = await _context.Folders
                .AsNoTracking()
                .Where(f => f.IsGlobal) // Chỉ lấy các folder có IsGlobal = true
                .Select(f =>
        ...
        public async Task<ServiceResult<List<ProofFileRes>>> GetAllFileInFolderAsync(Guid fileId)
        {
            try
            {
                var folders = await _context.ProofFiles
                    .AsNoTracking()
                    .Where(df => df.FolderId == fileId )  
                    .Select(g => new ProofFileRes
                    {
                        Id = g.Id,
                        FileName = g.FileName,
                ...
                return ServiceResult<List<ProofFileRes>>.Ok(folders);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to get document folders");
                return ServiceResult<List<ProofFileRes>>.Fail("Failed to get folders");
            }
        }
